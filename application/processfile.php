...
function validate($a,$submID,$username){
    $query = "SELECT DATE_FORMAT(`end`,'%Y-%m-%d %T') AS endTime,DATE_FORMAT(`start`,'%Y-%m-%d %T') AS startTime FROM contest WHERE contestID=$a";
    ...
    if($ctime<$stime){
        echo"Illegal";
        $query="UPDATE status SET status = 4,runtime=-1 where id = $submID";
        echo $query;
...
if($_SERVER["HTTP_coderunning"]==1){
	...
	$query="SELECT * from status where status=6 order by id";
...
if($verdict==0){
	$query="UPDATE status SET status = $verdict,score=$current_score,runtime=$runtime where id = $submID";
	mysql_query($query);

	$query="update problem set ts=ts+1,as=as+1 where problemid=$problemID";		   
	mysql_query($query);
	if($sts==1){
	...
	$query="UPDATE status SET status = $verdict,score=$current_score where id = $submID";
	mysql_query($query);
	echo $query;
	$query="update problem set ts=ts+1 where problemID=$problemID"; 
	mysql_query($query);
}
